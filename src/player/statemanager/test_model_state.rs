...
fn test_model_state_evaluation() {
    ...
    let result2 =
        state_machine.process_model_state_update("paused_model", &paused_model_containers);
    ...
    if result1.is_success() {
        ...
        match result1.new_state {
            ...
            _ => println!(
                "❌ test_model state was {}, expected Dead (4)",
                result1.new_state
            ),
        }
    } else {
    ...
    if result2.is_success() {
        ...
        match result2.new_state {
            ...
            _ => println!(
                "❌ paused_model state was {}, expected Paused (2)",
                result2.new_state
            ),
        }
    } else {
        println!(
            "❌ paused_model state transition failed: {}",
            result2.message
        );
    }
